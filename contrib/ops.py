...
class Return(Rolling):
    ...
    Calculates the return over the rolling window.
    """
    def __init__(self, feature, N):
        super(Return, self).__init__(feature, N, "return")
        
    def _load_internal(self, instrument, start_index, end_index, *args):
        series = self.feature.load(instrument, start_index, end_index, *args)
        if self.N == 0:
            return series / series.iloc[0] - 1
        return series / series.shift(self.N) - 1
...
class WMA(Rolling):
    ...
    def _load_internal(self, instrument, start_index, end_index, *args):
        ...
        if self.N == 0:
            def weighted_mean(x):
                w = np.arange(len(x)) + 1
                return np.nansum((w / w.sum()) * x)
            series = series.expanding(min_periods=1).apply(weighted_mean, raw=True)
        else:
            # Optimized WMA using convolution instead of apply
            w = np.arange(self.N) + 1.0
            w = w / w.sum()
            
            def rolling_wma(arr, weights):
                res = np.full_like(arr, np.nan, dtype=np.float64)
                for i in range(len(weights) - 1, len(arr)):
                    window = arr[i - len(weights) + 1: i + 1]
                    if not np.isnan(window).all():
                        res[i] = np.nansum(window * weights)
                return res

            res_arr = rolling_wma(series.values, w)
            series = pd.Series(res_arr, index=series.index)
        return series


class DecayLinear(Rolling):
    """DecayLinear Operator
    Calculates a linearly decaying weighted average over the rolling window.
    Weights are N, N-1, N-2... 1.
    """
    def __init__(self, feature, N):
        super(DecayLinear, self).__init__(feature, N, "decaylinear")

    def _load_internal(self, instrument, start_index, end_index, *args):
        series = self.feature.load(instrument, start_index, end_index, *args)
        
        if self.N == 0:
            def decay_linear_expanding(x):
                w = np.arange(len(x), 0, -1)
                return np.nansum((w / w.sum()) * x)
            series = series.expanding(min_periods=1).apply(decay_linear_expanding, raw=True)
        else:
            w = np.arange(self.N, 0, -1).astype(float)
            w = w / w.sum()
            
            def rolling_decay(arr, weights):
                res = np.full_like(arr, np.nan, dtype=np.float64)
                for i in range(len(weights) - 1, len(arr)):
                    window = arr[i - len(weights) + 1: i + 1]
                    if not np.isnan(window).all():
            ...
            res_arr = rolling_decay(series.values, w)
            series = pd.Series(res_arr, index=series.index)
        return series
