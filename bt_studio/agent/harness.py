...
class PrefilterResult:
    ...
    def to_summary(self) -> dict:
        return {
            "feature": self.feature_name,
            "passed": self.passed,
            "std": round(self.std, 8) if not np.isnan(self.std) else 0.0,
...
def evaluate_feature(
    ...
    try:
        ...
        if result and result.get("status") == "success":
            # fixbug None avoid score=0.0 / u_pavl=0.0 
            raw_score = result.get("metrics_score")
            raw_pval = result.get("u_pval")
            
            return EvalResult(
                feature_name=feature_col,
                status="success",
                score=float(raw_score) if raw_score is not None else -500.0,
                u_pval=float(raw_pval) if raw_pval is not None else 1.0,
...
def _evaluate_physical_stats(
    ...
    reasons = []
    # fixbug avoid NaN
    if np.isnan(std_val) or std_val < cfg["min_std"]:
        reasons.append(f"std={std_val} < {cfg['min_std']:.0e} (dead water or nan)")
...
class TwoStageAgentHarness:
        ...
        for idx, candidate in enumerate(candidate_asts):
            ...
            try:
                if isinstance(candidate, list):
                    ...
                    base_name = base_name or f"ast_feat_{idx}"

                # fixbug column conflict
                feat_name = base_name
                collision = 1
                ...
                if isinstance(candidate, list):
                    if feat_name != base_name:
                        hf_lf = hf_lf.rename({base_name: feat_name})
                else:
    ...
    def _persist_features(
        ...
        # fixbug ensure day columne
        required_cols = {"day"} | set(BASE_MARKET_COLS)
        base_cols = [c for c in required_cols if c in available_schema]
        ...
        for feat in winners:
            paths = []
            for m in months:
                p = os.path.join(FEATURE_DIR, f"hf_{feat}_{m}.parquet")
