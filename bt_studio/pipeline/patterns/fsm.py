...
#             .agg(pl.col(r_col).median().alias("median_ret"))
#             .drop_nulls()
#             .sort(b_col)
#         )
...
def evaluate_and_build_fsm(
    ...
    daily_macro_lf = (
        ...
        .with_columns([
            pl.col("daily_ofi_mean")
              .rolling_quantile(quantile=0.33, window_size=f"{rank_window}d", closed="left")
              .alias("p33"),
            pl.col("daily_ofi_mean")
              .rolling_quantile(quantile=0.67, window_size=f"{rank_window}d", closed="left")
              .alias("p67")
        ])
        .with_columns(
            pl.when(pl.col("daily_ofi_mean") <= pl.col("p33")).then(0)
    ...
    if not bin_cols:
        return {"status": "failed", "reason": "未匹配到任何有效的收益率 Z-Score 列", "metrics_score": -9999.0}
    ...
    if triggers.height < 5:
        return {"status": "failed", "reason": f"DTW 触发样本不足 (n={triggers.height})", "metrics_score": -9999.0}
    ...
    n_triggers = len(cond_ranks)
    if n_triggers < 30 or np.std(cond_ranks) < 1e-8 or len(uncond_ranks) == 0:
         return {"status": "failed", "reason": f"有效触发数({n_triggers})不足30或方差坍塌", "metrics_score": -9999.0}
    ...
    # =================================================================
    cond_rets = triggers["ret_trajectory"].drop_nulls().to_numpy()
    uncond_rets = complementary_df["ret_trajectory"].drop_nulls().to_numpy() # 同步切换为互补集
    ...
    if score <= -9990.0:
        return {"status": "failed", "reason": f"HPO得分熔断(P-val={u_pval:.4f})", "metrics_score": -9999.0}
